...
def get_mfcc_feature(wavsignal, fs):
    ...
    :return:
    """
    feat_mfcc = mfcc(wavsignal, fs, nfft=2048, nfilt=40, numcep=40, winlen=0.05, winstep=0.025)
    feat_mfcc_d = delta(feat_mfcc, 2)
    feat_mfcc_dd = delta(feat_mfcc_d, 2)
    wav_feature = np.column_stack((feat_mfcc, feat_mfcc_d, feat_mfcc_dd))
    return wav_feature

def get_mfcc_feature2(y, sr):
    mfcc = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=40)
    mfcc_d = librosa.feature.delta(mfcc, order=1)
    mfcc_dd = librosa.feature.delta(mfcc, order=2)
    return np.concatenate(
        (np.expand_dims(minmax_scale(mfcc).T, axis=0),
        np.expand_dims(minmax_scale(mfcc_d).T, axis=0),
        np.expand_dims(minmax_scale(mfcc_dd).T, axis=0)),
        axis=0
    )
    ...
    for root, dirs, files in os.walk(root_path, topdown=False):
        for name in files:
            ...
            for i in range(0, 27, 3):
                y, sr = librosa.load(filename, offset=i, duration=3)
...
print(np.load('concat_logbank.npy').shape) # (8973, 120, 120)
print(np.load('pre_process_mfcc.npy').shape) # (8973, 130, 40)
print(np.load('target.npy').shape)   # (8973,)
